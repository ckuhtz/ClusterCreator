...
#   bios = "ovmf"
  dynamic "disk" {
    ...
    content {
      file_id       = disk.value.index == 0 ? proxmox_virtual_environment_download_file.talos_nocloud_image[var.cluster_name].id : null # Set file_id only if it's the first disk
